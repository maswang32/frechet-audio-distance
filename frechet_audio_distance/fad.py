...
class FrechetAudioDistance:
    def __init__(
        ...
        if self.device == torch.device("mps") and self.model_name == "clap":
            if self.verbose:
                print(
                    "[Frechet Audio Distance] CLAP does not support MPS device yet, because:"
                )
                print(
                    "[Frechet Audio Distance] The operator 'aten::upsample_bicubic2d.out' is not currently implemented for the MPS device."
                )
    ...
    def __get_model(
        ...
        if model_name == "vggish":
            ...
            self.model = torch.hub.load(
                repo_or_dir="harritaylor/torchvggish", model="vggish"
            )
            if not use_pca:
        ...
        # clap
            ...
            if self.submodel_name in ["630k-audioset", "630k"]:
                self.model = laion_clap.CLAP_Module(
                    enable_fusion=self.enable_fusion, device=self.device
                )
            elif self.submodel_name in [
                "music_audioset",
                "music_speech",
                "music_speech_audioset",
            ]:
        ...
        try:
            for audio in tqdm(x, disable=(not self.verbose)):
                if self.model_name == "vggish":
                    ...
                    embd = self.model.get_audio_embedding_from_data(
    ...
    def score(
        ...
        - background_paths (List[str]): List of paths of the background audio files.
        - eval_paths (List[str]): List of paths of the evaluation audio files.
        ...
        try:
            ...
            if background_embds_path is not None and os.path.exists(
                ...
                if self.verbose:
                    print(
                        f"[Frechet Audio Distance] Loading embeddings from {background_embds_path}..."
                    )
                embds_background = np.load(background_embds_path)
            else:
                if background_paths is None:
                    audio_background = self.__load_audio_files(
                        dir=background_dir, dtype=dtype
                    )
                else:
                    audio_background = self.__load_audio_files(
            ...
            if eval_embds_path is not None and os.path.exists(eval_embds_path):
                if self.verbose:
                    print(
                        f"[Frechet Audio Distance] Loading embeddings from {eval_embds_path}..."
                    )
            ...
            if len(embds_background) == 0:
                print(
                    "[Frechet Audio Distance] background set dir is empty, exiting..."
                )
                return -1
            if len(embds_eval) == 0:
                print(
